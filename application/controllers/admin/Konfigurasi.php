...
class Konfigurasi extends CI_Controller
{
	...
	public function setting()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$data = array(
				'title'	=> 'Buka/Tutup Pendaftaran untuk Umum',
				'site'	=> $site,
	...
	// New CiptaSolusipratama
	public function CiptaSolusipratama()
	{
		$site = $this->konfigurasi_model->listing();
		...
		if ($v->run()) {
			if (!empty($_FILES['gambar']['name'])) {
				...
				if (!$this->upload->do_upload('gambar')) {
					...
					$data = array(
						'title'	=> $site['namaweb'] . ' Information',
						'site'	=> $site,
						'error'	=> $this->upload->display_errors(),
						'isi'	=> 'admin/konfigurasi/CiptaSolusipratama'
					);
					$this->load->view('admin/layout/wrapper', $data);
				} else {
					$upload_data				= array('uploads' => $this->upload->data());
					...
					$data = array(
						...
						'video'			=> $i->post('video'),
						'CiptaSolusipratama'			=> $i->post('CiptaSolusipratama'),
						'id_user'		=> $this->session->userdata('id')
					);
					$this->konfigurasi_model->edit($data);
					$this->session->set_flashdata('sukses', $site['namaweb'] . ' information changed');
					redirect(base_url('admin/konfigurasi/CiptaSolusipratama'));
				}
			} else {
				$i = $this->input;
				$data = array(
